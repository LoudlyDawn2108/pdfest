...
class LibraryDB:
    ...
    def update_chat_session(
        self, session_id, metadata_json, preview=None, title=None, messages_json=None
    ):
...
class AIPanelManager:
    ...
    def __init__(self, app):
        ...
        # Image context for visual selections (diagrams, equations, etc.)
        self.image_context_path = None
    ...
    def initialize_async(self, callback=None):
        ...
        if not GEMINI_AVAILABLE:
            if callback:
                self.app.root.after(
                    0, lambda: callback(False, "Gemini library not available")
                )
            return
        ...
        def init_thread():
            try:
                ...
                self._run_async(
                    self.client.init(timeout=300, auto_close=False, auto_refresh=True)
                )
    ...
    <chapter_header>{ctx.get("chapter_header", "N/A")}</chapter_header>
        ...
        if self.context_type == "selection":
            # Check if we have an image context attached
            has_image = self.image_context_path is not None
            image_note = (
                """
    <attached_image>
    An image of the selected area from the PDF is attached to this message.
    Please analyze both the image and the text context provided.
    </attached_image>"""
                if has_image
                else ""
            )
...
Question: {message}"""
    ...
    def send_message_async(self, message, callback=None):
        ...
        # Capture image path for use in thread (may be cleared after)
        image_path = self.image_context_path
        ...
        def send_thread():
            try:
                ...
                # Send with image if present, otherwise just text
                import os

                if image_path and os.path.exists(image_path):
                    response = self._run_async(
                        self.chat_session.send_message(prompt, files=[image_path])
                    )
                else:
    ...
    def set_rich_context(
        ...
        """Set rich context with surrounding text and metadata, optionally with an image"""
        ...
        # Clean up old image if exists before setting new one
        self._cleanup_image_context()
        self.image_context_path = image_path
    ...
    def _cleanup_image_context(self):
        """Clean up temporary image context file"""
        if self.image_context_path:
            try:
                import os

                if os.path.exists(self.image_context_path):
                    os.remove(self.image_context_path)
            except Exception:
    ...
    def save_current_session(self):
        ...
        first_user_msg = next(
            (msg for role, msg in self.conversation_history if role == "user"),
            "New Chat",
        )
        title = (
        ...
        if self.current_session_id is None:
            # Create new session
            self.current_session_id = self.app.db.save_chat_session(
                title, metadata_json, preview, messages_json
            )
        else:
            # Update existing session
            self.app.db.update_chat_session(
    ...
    def load_session(self, session_id):
        ...
        if metadata and self.client:
            self.chat_session = self.client.start_chat(
                metadata=metadata, model=Model.G_3_0_PRO
            )
...
class VisualEdgeReader:
    def __init__(self, root):
        ...
        self.image_selection_mode = False  # True when Alt is held during selection
        ...
        ttk.Button(toolbar, text="📚 Library", command=self.show_library).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(toolbar, text="📖 Open", command=self.open_document).pack(
            side=tk.LEFT, padx=5
        )

        # Sidebar toggle
        ttk.Button(toolbar, text="☰ TOC", command=self.toggle_sidebar).pack(
            side=tk.LEFT, padx=5
        )

        # Margin settings (for TTS exclusion)
        ttk.Button(toolbar, text="📏 Margins", command=self.show_margin_settings).pack(
            side=tk.LEFT, padx=5
        )

        # Brightness control
        ttk.Button(toolbar, text="🌙 Dim", command=self.show_brightness_settings).pack(
            side=tk.LEFT, padx=5
        )

        # Column mode toggle (1-col / 2-col)
        self.btn_column_mode = ttk.Button(
            toolbar, text="📄 1-Col", command=self.toggle_column_mode
        )
        ...
        ttk.Button(toolbar, text=">> Next Sent", command=self.next_sentence).pack(
            side=tk.RIGHT
        )
        ttk.Button(toolbar, text="<< Prev Sent", command=self.prev_sentence).pack(
            side=tk.RIGHT
        )

        # Voice selection (right side, next to playback)
        ttk.Button(toolbar, text="🔊 Voice", command=self.show_voice_settings).pack(
        ...
        ttk.Button(page_frame, text="−", width=3, command=self.zoom_out).pack(
            side=tk.LEFT, padx=2
        )
        ...
        ttk.Button(page_frame, text="+", width=3, command=self.zoom_in).pack(
            side=tk.LEFT, padx=(2, 15)
        )
        ...
        self.lbl_total_pages = tk.Label(
            page_frame, text=" of 0", bg="#2d2d30", fg="white", font=("Arial", 10)
        )
        ...
        self.resize_handle = tk.Frame(
            self.main_frame, bg="#444", width=5, cursor="sb_h_double_arrow"
        )
    ...
    def open_document(self, filename=None):
        ...
        if book_info:
            ...
            self.header_margin = float(
                book_info["header_margin"]
                if book_info["header_margin"] is not None
                else 50
            )
            self.footer_margin = float(
                book_info["footer_margin"]
                if book_info["footer_margin"] is not None
                else 60
            )
            # Load column mode
            self.column_mode = int(
                book_info["column_mode"] if book_info["column_mode"] is not None else 1
            )
        ...
        self.canvas.config(
            scrollregion=(
                0,
                0,
                max(self.page_width, self.canvas.winfo_width()),
                self.canvas_height,
            )
        )
    ...
    def _create_ai_panel(self):
        ...
        self.ai_resize_handle = tk.Frame(
            self.main_frame, bg="#444", width=5, cursor="sb_h_double_arrow"
        )
        ...
        self.ai_panel = tk.Frame(
            self.main_frame, bg="#2d2d30", width=self.ai_panel_width
        )
        ...
        ttk.Button(ai_header, text="✕", width=3, command=self.toggle_ai_panel).pack(
            side=tk.RIGHT, padx=5
        )
        ttk.Button(ai_header, text="📋", width=3, command=self.show_chat_history).pack(
            side=tk.RIGHT
        )
        ttk.Button(ai_header, text="➕", width=3, command=self.new_ai_chat).pack(
            side=tk.RIGHT
        )
        ...
        ttk.Button(
            ...
            text="📄 Page",
            width=7,
            command=self.use_current_page_context,
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            context_btn_frame, text="✕", width=3, command=self.clear_ai_context
        ).pack(side=tk.LEFT)

        # Then pack label to fill remaining space
        self.ai_context_label = tk.Label(
        ...
        self.ai_chat_canvas.bind(
            "<Button-4>", self._on_ai_chat_mousewheel
        )  # Linux scroll up
        self.ai_chat_canvas.bind(
            "<Button-5>", self._on_ai_chat_mousewheel
        )  # Linux scroll down

        self.ai_chat_frame = tk.Frame(self.ai_chat_canvas, bg="#1a1a1a")
        self.ai_chat_window_id = self.ai_chat_canvas.create_window(
            (0, 0), window=self.ai_chat_frame, anchor=tk.NW
        )
        ...
        self._add_ai_message(
            "ai",
            "Hello! I'm your AI assistant. Select text or use the current page as context, then ask me any questions about what you're reading.",
        )
        ...
        self.ai_send_btn = ttk.Button(
            send_frame, text="Send", command=self.send_ai_query
        )
    ...
    def _do_ai_panel_resize(self, event):
        ...
        delta = (
            self.ai_resize_start_x - event.x_root
        )  # Reversed because panel is on right
    ...
    def toggle_ai_panel(self):
        ...
        if self.ai_panel_visible:
            ...
            if (
                not self.ai_manager.is_initialized
                and not self.ai_manager.is_initializing
            ):
            ...
            try:
                ...
                try:
                    ...
                    if display_lines:
                        ...
                        num_lines = max(
                            1, (len(message) // widget_width) + message.count("\n") + 1
                        )
                except:
        ...
        def on_init_complete(success, error):
            if success:
                self.ai_status_label.config(
                    text="Ready! Ctrl+Enter to send", fg="#4CAF50"
                )
            else:
                self.ai_status_label.config(
                    text=f"Error: {error[:50]}...", fg="#f44336"
                )
                self._add_ai_message(
                    "ai",
                    f"⚠️ Failed to connect to Gemini: {error}\n\nMake sure you're logged into gemini.google.com in your browser.",
                )
    ...
    def update_ai_context_rich(
        self, selection, preceding_text, following_text, page_num
    ):
    ...
    def update_ai_context_image(
        self, image_path, selection_text, preceding_text, following_text, page_num
    ):
        """Update AI context with an image selection plus surrounding text"""
        # Get document metadata
        doc_title, doc_author = self._get_document_metadata()
        chapter_header = self._get_chapter_for_page(page_num)
        ...
        # Update context label to show image mode
        self.ai_context_label.config(
            text=f"🖼️ Image selection (Page {page_num + 1})", fg="#88ff88"
        )
    ...
    def _get_document_metadata(self):
        ...
        title = (
            metadata.get("title", "") or Path(self.current_pdf_path).stem
    ...
    def use_current_page_context(self):
        ...
        if text.strip():
            ...
            self.ai_context_label.config(
                text="📄 No text found on this page", fg="#ff6b6b"
            )
    ...
    def show_chat_history(self):
        ...
        for session in sessions:
            ...
            display_text = (
                f"{title[:40]}... ({date})" if len(title) > 40 else f"{title} ({date})"
            )
        ...
        ttk.Button(btn_frame, text="Load", command=load_selected).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def _load_chat_session(self, session_id):
        ...
        if success:
            ...
            if messages:
                ...
                self._add_ai_message(
                    "ai",
                    "Previous conversation loaded. Continue chatting or ask a new question.",
                )
        else:
    ...
    def toggle_column_mode(self):
        ...
        for page_num in self.loaded_pages:
            page = self.doc.load_page(page_num)
            y_offset = self.page_offsets.get(
                page_num, page_num * self.estimated_page_height
            )
    ...
    def render_single_page(self, page_num):
        ...
        self.page_pil_images[page_num] = (
            img_data.copy()
        )  # Store original (full brightness)
        ...
        self.canvas.create_image(
            x_offset, y_offset, image=photo, anchor=tk.NW, tags=f"page_{page_num}"
        )
    ...
    def analyze_page_sentences(self, page, page_num, y_offset):
        ...
        if self.column_mode == 2:
            ...
            right_words = [
                w for w in filtered_words if w[0] >= midpoint
            ]  # x0 >= midpoint
        ...
        if current_text:
            self.sentences.append(
                PDFSentence(" ".join(current_text), current_rects, page_num, y_offset)
            )
        ...
        if self.column_mode == 2:
            ...
            self.sentences.sort(
                key=lambda s: (
                    s.page_num,
                    0
                    if (s.rects and s.rects[0][0] < midpoint)
                    else 1,  # left=0, right=1
                    s.rects[0][1] if s.rects else 0,  # Y position within column
                )
            )
        else:
            self.sentences.sort(
                key=lambda s: (s.page_num, s.rects[0][1] if s.rects else 0)
            )
    ...
    def check_and_load_more_pages(self):
        ...
        if visible_page >= max_loaded - LOAD_THRESHOLD + 1:
            ...
            for p in range(
                next_start, min(next_start + PAGES_PER_BATCH, self.total_pages)
            ):
                if p not in self.loaded_pages:
    ...
    def _animate_scroll(self, steps_remaining):
        ...
        if abs(self._scroll_queue) > 0.5:
            self.root.after(
                12, lambda: self._animate_scroll(steps_remaining - 1)
            )  # ~83fps
        else:
            self._scroll_animating = False
            self._scroll_queue = 0
    ...
    def on_selection_start(self, event):
        ...
        # Check if Alt key is held for image selection mode
        # Alt key: 0x20000 on Linux, 0x8 on some systems
        self.image_selection_mode = (event.state & 0x20000) != 0 or (
            event.state & 0x8
        ) != 0
    ...
    def on_selection_drag(self, event):
        ...
        # Use green for image mode, blue for text mode
        self.canvas.delete("selection_rect")
        x0, y0 = self.selection_start
        x1, y1 = self.selection_end

        # Check current Alt key state (may change during drag)
        is_image_mode = (event.state & 0x20000) != 0 or (event.state & 0x8) != 0
        self.image_selection_mode = is_image_mode

        if is_image_mode:
            fill_color = "#44ff88"  # Green for image mode
            outline_color = "#22dd66"
        else:
            fill_color = "#4488ff"  # Blue for text mode
            outline_color = "#2266dd"
    ...
    def check_and_open_link(self, canvas_x, canvas_y):
        ...
        for page_num in self.loaded_pages:
            ...
            for link in links:
                ...
                if (
                    rect
                    and rect.x0 <= page_x <= rect.x1
                    and rect.y0 <= page_y <= rect.y1
                ):
                    # Found a link!
                    uri = link.get("uri")
                    if uri:
                    ...
                    # dest_page can be int or string depending on document type
                    if dest_page is not None:
                        try:
                            dest_page_int = int(dest_page)
                            if dest_page_int >= 0:
                                self.scroll_to_page(dest_page_int)
                                return
    ...
    def extract_and_highlight_selection(self):
        """Extract text from selection area and draw highlights, or capture as image"""
        ...
        # For image mode, we need to capture a single image from the first page
        image_capture_rect = None
        image_capture_page = None
        ...
        for page_num in self.loaded_pages:
            ...
            page = self.doc.load_page(page_num)
            rect = fitz.Rect(
                page_sel_left, page_sel_top, page_sel_right, page_sel_bottom
            )

            # Store rect for image capture (only first page)
            if image_capture_rect is None:
                image_capture_rect = rect
                image_capture_page = page
            ...
            is_image_mode = getattr(self, "image_selection_mode", False)
            highlight_color = "#44ff88" if is_image_mode else "#4488ff"
        ...
        # Check if we're in image selection mode
        is_image_mode = getattr(self, "image_selection_mode", False)

        # Update AI context with rich surrounding context
        if selection_page_num is not None:
            ...
            if (
                is_image_mode
                and image_capture_page is not None
                and image_capture_rect is not None
            ):
                # Image mode: capture selection as image
                image_path = self._capture_selection_image(
                    image_capture_page, image_capture_rect, selection_page_num
                )
                if image_path:
                    self.update_ai_context_image(
                        image_path,
            ...
            elif self.selected_text.strip():
                # Text mode: use text context
    ...
    def _capture_selection_image(self, page, rect, page_num):
        """Capture the selected area as an image and save to temp file"""
        try:
            import os
            import time

            # Create temp directory if it doesn't exist
            temp_dir = Path.home() / ".local" / "pdfest" / "temp"
            temp_dir.mkdir(parents=True, exist_ok=True)

            # Generate unique filename
            timestamp = int(time.time() * 1000)
            image_path = temp_dir / f"selection_{page_num}_{timestamp}.png"
            ...
            print(f"Error capturing selection image: {e}")
            return None

    def _get_surrounding_text(
        self, page_num, sel_top_canvas, sel_bottom_canvas, x_offset
    ):
        ...
        following_rect = fitz.Rect(
            0, min(page_height, sel_bottom_page), page_width, page_height
        )
    ...
    def draw_highlight(self):
        ...
        if (
            self.last_highlighted_page is not None
            and self.last_highlighted_page != page_num
        ):
    ...
    def toggle_play(self):
        if self.is_playing:
            ...
            visible_page = self.get_visible_page()
            current_sentence = (
                self.sentences[self.current_sentence_idx] if self.sentences else None
            )
    ...
    def _cache_worker(self):
        ...
        try:
            while self.is_playing and not self.stop_signal:
                ...
                for i in range(
                    current, min(current + self.cache_ahead, len(self.sentences))
                ):
    ...
    def playback_loop(self):
        ...
        while self.current_sentence_idx < len(self.sentences) and not self.stop_signal:
            ...
            if sentence.page_num not in self.loaded_pages:
                self.root.after(
                    0, lambda pn=sentence.page_num: self.render_single_page(pn)
                )
    ...
    def show_margin_settings(self):
        ...
        tk.Label(
            margin_win,
            text="Exclude header/footer regions from TTS reading",
            bg="#2d2d30",
            fg="#aaa",
            font=("Arial", 10),
        ).pack()
        ...
        header_scale.bind(
            "<Motion>",
            lambda e: self.preview_margins(header_var.get(), footer_var.get()),
        )
        ...
        ttk.Button(btn_frame, text="Apply", command=apply_margins).pack(
        ...
        for page_num in self.loaded_pages:
            ...
            if header > 0:
                self.canvas.create_rectangle(
                    x_offset,
                    y_offset,
                    x_offset + self.page_width,
                    y_offset + header * self.zoom_level,
                    fill="#ff4444",
                    stipple="gray50",
                    outline="#ff0000",
                    tags="margin_preview",
    ...
    def show_brightness_settings(self):
        ...
        tk.Label(
            bright_win,
            text="Lower brightness to reduce eye strain",
            bg="#2d2d30",
            fg="#aaa",
            font=("Arial", 10),
        ).pack()
        ...
        tk.Label(
            slider_frame, text="🔅", bg="#2d2d30", fg="white", font=("Arial", 14)
        ).pack(side=tk.LEFT)
    ...
    def show_library(self):
        ...
        ttk.Button(
            header,
            text="+ Add Book",
            command=lambda: self._add_book_to_library(library_win),
        ...
        def refresh_book_list(*args):
            ...
            filtered_books = [
                b
                for b in all_books
                if query in (b["title"] or "").lower() or query in b["path"].lower()
            ]
            ...
            if not filtered_books:
                ...
                for book in filtered_books:
                    self._create_book_card(
                        scrollable_frame, book, library_win, on_library_scroll
                    )
    ...
    def _create_book_card(self, parent, book, library_win, scroll_handler=None):
        ...
        progress = (
            f"Page {book['last_page'] + 1} of {book['total_pages']}"
        ...
        ttk.Button(
            btn_frame,
            text="Open",
            command=lambda p=book["path"]: self._open_from_library(p, library_win),
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            btn_frame,
            text="Remove",
            command=lambda p=book["path"], c=card: self._remove_from_library(p, c),
        ).pack(side=tk.LEFT)
        ...
        for widget in clickable_widgets:
            widget.bind(
                "<Button-1>",
                lambda e, p=book["path"]: self._open_from_library(p, library_win),
            )
    ...
    def show_voice_settings(self):
        ...
        def load_voices():
            try:
                ...
                voice_win.after(
                    0,
                    lambda: voice_listbox.insert(tk.END, f"Error loading voices: {e}"),
                )
        ...
        def apply_voice():
            ...
            if selection:
                ...
                if (
                    ...
                    or selected_text == "Loading voices..."
                ):
                    return
                # Extract voice name
        ...
        ttk.Button(btn_frame, text="Apply", command=apply_voice).pack(
            side=tk.RIGHT, padx=20
        )
        ttk.Button(btn_frame, text="Cancel", command=voice_win.destroy).pack(
            side=tk.RIGHT
        )
